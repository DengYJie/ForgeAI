...
bool QLiteHtmlWidget::eventFilter(QObject* obj, QEvent* event)
{
    if (!d->ignoreScrollbarWheel && (obj == verticalScrollBar() || obj == horizontalScrollBar()) && event->type() == QEvent::Wheel) {
        ...
        QWheelEvent clonedEvent(
            viewport()->mapFromGlobal(wheelEvent->globalPosition()),
...
void QLiteHtmlWidget::updateHightlightedLink()
{
    QPoint viewportPos;
    QPoint pos;
    htmlPos(viewport()->mapFromGlobal(QCursor::pos()), &viewportPos, &pos);
...
void QLiteHtmlWidget::htmlPos(const QPoint& viewportPoint, QPoint* viewportPos, QPoint* htmlPos) const
{
    *viewportPos = toVirtual(viewportPoint);
