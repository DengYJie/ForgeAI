...
namespace ui::screen::chat {
    ...
    QRect ChatSessionDelegate::deleteButtonRect(const QRect &itemRect) const {
        const int y = itemRect.top() + (itemRect.height() - kButtonSize) / 2;
    ...
    void ChatSessionDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
        ...
        const QRect delRect = deleteButtonRect(option.rect);

        if (showButtons || isPinned) {
            // Pin 按钮
        ...
        if (showButtons) {
            // Delete 按钮
            if (delRect.contains(m_hoveredPos)) {
                painter->setBrush(colors.subtleSecondary);
                painter->setPen(Qt::NoPen);
                painter->drawRoundedRect(delRect, 3, 3);
            ...
            painter->setFont(QFont(Typography::FontFamily::FluentIcons, 10));
            painter->drawText(delRect, Qt::AlignCenter, Typography::Icons::ChromeClose);
        }
